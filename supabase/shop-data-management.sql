-- Version 2 Shop data-backup restore support.
-- Restores only missing rows into the same shop and never overwrites existing data.
...
begin
    ...
    if not public.has_shop_role(target_shop, array['owner', 'admin']) then
